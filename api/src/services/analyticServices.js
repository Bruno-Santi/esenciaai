...
analyticServices.createRetro = async(team_id, user_id, retroList ) => {
    console.log(team_id)
    const existTeam = await UserTeam.findOne({where: { teamId: team_id, role: "admin", userId: user_id }})
    if(!existTeam) throwError(
      "access_denied",
    403,
