...
export const useDashboard = () => {
  ...
  const { userTeams, activeTeam, membersActiveTeam } = useSelector(
  ...
  useEffect(() => {
    ...
    if (localTeams) {
      ...
      const uniqueLocalTeams = parsedLocalTeams.filter(
        (localTeam) =>
          !filteredUserTeams.some(
            (filteredTeam) => filteredTeam.id === localTeam.id
          )
      );
      ...
      dispatch(
        onSetUserTeams({
          userTeams: mergedTeams,
        })
      );
    } else {
      dispatch(
        onSetUserTeams({
          userTeams: filteredUserTeams,
        })
      );
    }
    if (!localTeams) {
  ...
  const startCreatingTeam = async (newTeam: UserTeams) => {
    ...
    team.logo = newTeam.logo || teamLogo;
    console.log(team);
    try {
  ...
  const startSettingActiveMembers = (id) => {
    ...
    const membersById: Members[] = TeamMembers.filter(
